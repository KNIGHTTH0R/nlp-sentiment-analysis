...
path = '/Users/aaron/Documents/work_python/nlp-sentiment-analysis/cnn'
...
def freq_factor(texts, percentage=0.85, savefile=None, drawable=False):
    ...
    len_count_list = sorted(len_count_map.items(),
    ...
    for item in len_count_list:
        ...
        if total / sum(counts) > percentage:
            # 将长度-出现频率按照长度进行排序
            accept_length = sorted(
                accept_length, key=lambda d: d[0], reverse=False)
            print("出现次数最多的句子长度是:{}".format(keys[0]))
            print("出现次数：{}".format(counts[0]))
            print("句子长度在{}~{}占据所有数据的{}".format(accept_length[
                  0][0], accept_length[-1][0], percentage))
            print("句子长度为{},出现的次数为{}".format(
                accept_length[-1][0], accept_length[-1][1]))
            break
    ...
    if drawable:
        ...
        # 绘制在同一张图中
        plt.plot(keys, counts, color="blue", linewidth=1.0,
...
def load_data_from_dir(dir_path, data_dir):
    ...
    for f in neg_file_list:
        lines_list = [line.strip() for line in open(
            os.path.join(neg_dir, f)).readlines() if line.strip() != ""]
        file_one_line = ""
    ...
    for f in pos_file_list:
        lines_list = [line.strip() for line in open(
            os.path.join(pos_dir, f)).readlines() if line.strip() != ""]
        file_one_line = ""
        for line in lines_list:
            file_one_line = file_one_line + line
...
def trans_files_to_file():
    ...
    data_dir = ["neg", "pos"]
    # 获取原始的评论数据
    neg_examples, pos_examples, y_labels = load_data_from_dir(
        dir_path=dir_path, data_dir=data_dir)
    # 对原始数据进行分词
