...
if __name__ == "__main__":
    ...
    originData = sc.textFile(u'spide/output/速度与激情8 The Fate of the Furious.txt')
    ...
    accuracy_nb = 1.0 * predictionAndLabel.filter(lambda x: 1.0 if x[0] == x[1] else 0.0).count() / test.count()
    print accuracy_nb
    ...
    accuracy_svm = 1.0 * predictionAndLabel.filter(lambda x: 1.0 if x[0] == x[1] else 0.0).count() / test.count()
    print accuracy_svm
